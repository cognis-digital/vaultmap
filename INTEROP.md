...
How **vaultmap** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `vaultmap` lives in the **Identity / persona** cluster.
...
| from | relation | to |
|---|---|---|
| `vaultmap` | JSON findings compose with | [`personagraph`](https://github.com/cognis-digital/personagraph), [`corpmap`](https://github.com/cognis-digital/corpmap) |
| `vaultmap` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `vaultmap` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `vaultmap` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the identity / persona cluster.** `vaultmap` output feeds the next tool:
```bash
vaultmap ... --format json > out.json          # this tool's findings (see `vaultmap --help`)
...
vaultmap ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
vaultmap ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
vaultmap ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `vaultmap`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `vaultmap` slots
